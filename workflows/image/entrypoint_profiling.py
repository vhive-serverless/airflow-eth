...
def run_task_instance():
    ...
    airflow_stats = open("airflow_stats.txt", "w")
    p = subprocess.Popen(data["args"], stdout=airflow_stats, stderr=subprocess.PIPE)
    airflow_stats.close()
    timer.time("after_start_subprocess")
    p.wait()
    timer.time("after_finished_subprocess")
    logging.info(f"exitcode: {p.returncode}; stderr: {p.stderr.read()}")
