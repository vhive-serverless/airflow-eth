...
curl -u admin:admin -X POST -H 'application/json' --data '{"input": [1,2,3,4]}' "$GATEWAY_URL"/runWorkflow/producer_consumer
sleep 5
...
producer="$(kubectl -n airflow get pods | grep benchmark-w1-d2-extract | awk '{print $1}')"
kubectl -n airflow logs "$producer" user-container > "$log_dir"/log_producer.log
consumer="$(kubectl -n airflow get pods | grep enchmark-w1-d2-do-sum | awk '{print $1}')"
